...
@section('content')
    ...
    <section class="content">
        <div class="container-fluid">
            ...
            <a href="{{ route('admin.aboutCompanies.index') }}" title="Назад"
               class="btn btn-warning btn-sm mb-3">
                <i class="fa fa-arrow-left" aria-hidden="true"></i>
                Назад
            </a>
            <form method="POST" class="pb-5" action="{{ route('admin.aboutCompanies.store') }}"
                  enctype="multipart/form-data">
                @csrf
                @method('POST')
                @include('admin.aboutCompanies._form')
            </form>
        </div>
